...
class ContentCategoryController extends Controller
{
    ...
    public function getUserCategories($account_id, $catalog_id)
    {
        ...
        $categories = DB::table('content_categories')
                            ...
                            ->selectRaw("(select count(*) from `contents` where `contents`.`category_id` = `content_categories`.`id` and date(`updated_at`) > IFNULL((select category_visit_logs.created_at from category_visit_logs where category_visit_logs.category_id=contents.category_id AND category_visit_logs.customer_id = ? ORDER BY id DESC LIMIT 1), ?) and `contents`.`deleted_at` is null) as `contents_count`", [$customer->id, $default_last_visit_category_date])
